...
class AdminAuth:
    def __init__(self):
        self.username = setting("WEBSITE_ASSISTANT_ADMIN_USERNAME", "").strip()
        self.password_hash = setting("WEBSITE_ASSISTANT_ADMIN_PASSWORD_HASH", "")
        self.role = setting("WEBSITE_ASSISTANT_ADMIN_ROLE", "administrator").strip().lower()
        self.secure = setting("WEBSITE_ASSISTANT_ADMIN_COOKIE_SECURE", "true").lower() != "false"
        try:
            self.ttl = max(5, min(120, int(setting("WEBSITE_ASSISTANT_ADMIN_SESSION_MINUTES", "30")))) * 60
        except ValueError:
            self.ttl = 1800
        data_path = Path(setting("WEBSITE_ASSISTANT_DATA_PATH", "data"))
        self.database_path = Path(setting(
            "WEBSITE_ASSISTANT_ADMIN_SESSION_DB", str(data_path / "admin_sessions.db")))
        self.attempts = {}
